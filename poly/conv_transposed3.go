			...
			for b := 0; b < batchSize; b++ {
				for ic := 0; ic < inC; ic++ {
					for id := 0; id < inD; id++ {
						for ih := 0; ih < inH; ih++ {
							for iw := 0; iw < inW; iw++ {
								inputVal := float64(input.Data[b*inC*inD*inH*inW + ic*inD*inH*inW + id*inH*inW + ih*inW + iw])
								for f := 0; f < filters; f++ {
									for kd := 0; kd < kSize; kd++ {
										for kh := 0; kh < kSize; kh++ {
											for kw := 0; kw < kSize; kw++ {
												od := id*stride - padding + kd
			...
			for b := 0; b < batchSize; b++ {
				for ic := 0; ic < inC; ic++ {
					for id := 0; id < inD; id++ {
						for ih := 0; ih < inH; ih++ {
							for iw := 0; iw < inW; iw++ {
								inputVal := float32(input.Data[b*inC*inD*inH*inW + ic*inD*inH*inW + id*inH*inW + ih*inW + iw])
								for f := 0; f < filters; f++ {
									for kd := 0; kd < kSize; kd++ {
										for kh := 0; kh < kSize; kh++ {
											for kw := 0; kw < kSize; kw++ {
