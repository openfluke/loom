...
func (t *Transformer[T]) ForwardWGPU(input *Tensor[T]) (*Tensor[T], error) {
	...
	// 2. Open a single command encoder for ALL layer dispatches.
	//    This collapses ~150+ Queue.Submit calls per token down to 1.
	...
	// 3. Flush all layer commands in ONE submit.
	ctx.FlushFrame()

	// 4. Copy result to staging and download — this is a separate submit + async wait.
