...
func (c *WGPUContext) DispatchDenseI8(
	...
	pBuf := c.GetUniformBuffer(uint64(unsafe.Sizeof(params)))
	c.Queue.WriteBuffer(pBuf, 0, wgpu.ToBytes([]WGPUDenseI8Params{params}))
...
func (c *WGPUContext) DispatchSwiGLUI8(
	...
	pBuf := c.GetUniformBuffer(uint64(unsafe.Sizeof(params)))
	c.Queue.WriteBuffer(pBuf, 0, wgpu.ToBytes([]WGPUSwiGLUI8Params{params}))
...
func (c *WGPUContext) DispatchEmbedding(
	...
	for rowOff := 0; rowOff < vocabSize; rowOff += maxRows {
		...
		pBuf := c.GetUniformBuffer(uint64(unsafe.Sizeof(pEmbed)))
		c.Queue.WriteBuffer(pBuf, 0, wgpu.ToBytes([]WGPUEmbeddingShardParams{pEmbed}))
...
func (c *WGPUContext) DispatchFillZero(size int, buf *wgpu.Buffer) error {
	...
	pBuf := c.GetUniformBuffer(uint64(unsafe.Sizeof(p)))
	c.Queue.WriteBuffer(pBuf, 0, wgpu.ToBytes([]uint32{p.Size}))
...
func (c *WGPUContext) DispatchQuantizeI8(size int, scale float32, masterBuf, nativeBuf *wgpu.Buffer) error {
	...
	pBuf := c.GetUniformBuffer(uint64(unsafe.Sizeof(p)))
	c.Queue.WriteBuffer(pBuf, 0, wgpu.ToBytes([]WGPUQuantizeParams{p}))
	...
	pass.SetPipeline(pipeline)
	pass.SetBindGroup(0, bindGroup, nil)
	pass.DispatchWorkgroups((uint32(size/4)+63)/64, 1, 1)
	pass.End()
	ctxSubmit(c, enc, owned)
	return nil
}
...
func (c *WGPUContext) DispatchQuantizeI4(size int, scale float32, masterBuf, nativeBuf *wgpu.Buffer) error {
	...
	pass.SetPipeline(pipeline)
	pass.SetBindGroup(0, bindGroup, nil)
	pass.DispatchWorkgroups((uint32(size/8)+63)/64, 1, 1)
	pass.End()
	ctxSubmit(c, enc, owned)
	return nil
}
...
func (c *WGPUContext) DispatchQuantizeTernary(size int, scale float32, masterBuf, nativeBuf *wgpu.Buffer) error {
	...
	pass.SetPipeline(pipeline)
	pass.SetBindGroup(0, bindGroup, nil)
	pass.DispatchWorkgroups((uint32(size/16)+63)/64, 1, 1)
	pass.End()
	ctxSubmit(c, enc, owned)
	return nil
}
...
func (c *WGPUContext) DispatchQuantizeBinary(size int, scale float32, masterBuf, nativeBuf *wgpu.Buffer) error {
	...
	pass.SetPipeline(pipeline)
	pass.SetBindGroup(0, bindGroup, nil)
	pass.DispatchWorkgroups((uint32(size/32)+63)/64, 1, 1)
	pass.End()
	ctxSubmit(c, enc, owned)
	return nil
}
...
func (c *WGPUContext) DispatchMultiHeadSoftmaxCEGradPartialLoss(
	...
	pBuf := c.GetUniformBuffer(uint64(unsafe.Sizeof(p)))
	c.Queue.WriteBuffer(pBuf, 0, wgpu.ToBytes([]WGPUMultiHeadSoftmaxCEParams{p}))
