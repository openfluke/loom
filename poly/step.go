package poly

import (
	"sync"
	"time"
)

/*
STEP MESH (Discrete-Time Neural Mesh)
-------------------------------------
This technique treats the 3D Volumetric Grid as a living mesh.
Unlike standard sequential propagation where an input flows through the
entire network in one cycle, step mesh propagation processes every
coordinate simultaneously from its current input buffer.

Key Features:
- Clock-cycle accuracy: each tick moves data one layer deep through the grid.
- Double buffering: avoids races during simultaneous layer updates.
- Temporal feedback: remote links create discrete-time recurrence (RNN-like).
- Step backward: gradients flow backward through preserved temporal history.
*/

// StepState holds the temporal snapshot of the 3D grid.
type StepState[T Numeric] struct {
	// LayerData holds the current output of every layer in the grid.
	// Indexing follows VolumetricNetwork.GetIndex(z, y, x, l)
	LayerData []*Tensor[T]

	// BackwardContext stores pre-activations and inputs for backpropagation.
	// These are indexed by [Step][LayerIndex] to allow BPTT across clock cycles.
	HistoryIn  [][]*Tensor[T]
	HistoryPre [][]*Tensor[T]

	// Double buffering for simultaneous updates
	NextBuffer []*Tensor[T]

	// Grid Metadata
	StepCount uint64
	mu        sync.RWMutex

	// Neural Target Propagation Bridge
	TweenState *TweenState[T]
	lastInput  *Tensor[T]
}

// NewStepState initializes a state for a specific Volumetric Network.
func NewStepState[T Numeric](n *VolumetricNetwork) *StepState[T] {
	total := len(n.Layers)
	return &StepState[T]{
		LayerData:  make([]*Tensor[T], total),
		NextBuffer: make([]*Tensor[T], total),
		HistoryIn:  make([][]*Tensor[T], 0),
		HistoryPre: make([][]*Tensor[T], 0),
	}
}

// SetInput injects data into the starting coordinate (0,0,0,0).
func (s *StepState[T]) SetInput(input *Tensor[T]) {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.lastInput = input
	if len(s.LayerData) > 0 {
		s.LayerData[0] = input
	}
}

// StepForward executes one "Clock Cycle" across the entire 3D grid.
// Every layer processes its current input buffer and writes to the next buffer.
func StepForward[T Numeric](n *VolumetricNetwork, s *StepState[T], captureHistory bool) time.Duration {
	start := time.Now()
	s.mu.Lock()
	defer s.mu.Unlock()

	numLayers := len(n.Layers)
	var currentIn, currentPre []*Tensor[T]
	if captureHistory {
		currentIn = make([]*Tensor[T], numLayers)
		currentPre = make([]*Tensor[T], numLayers)
	}

	// Parallel tile-based dispatch: spatial 4×4×4 blocks, one goroutine per tile.
	tileSize := 4
	var wg sync.WaitGroup
	for zTile := 0; zTile < n.Depth; zTile += tileSize {
		zEnd := zTile + tileSize
		if zEnd > n.Depth {
			zEnd = n.Depth
		}
		for yTile := 0; yTile < n.Rows; yTile += tileSize {
			yEnd := yTile + tileSize
			if yEnd > n.Rows {
				yEnd = n.Rows
			}
			for xTile := 0; xTile < n.Cols; xTile += tileSize {
				xEnd := xTile + tileSize
				if xEnd > n.Cols {
					xEnd = n.Cols
				}

				wg.Add(1)
				go func(zT, zE, yT, yE, xT, xE int) {
					defer wg.Done()
					for z := zT; z < zE; z++ {
						for y := yT; y < yE; y++ {
							for x := xT; x < xE; x++ {
								for lIdx := 0; lIdx < n.LayersPerCell; lIdx++ {
									idx := n.GetIndex(z, y, x, lIdx)
									l := &n.Layers[idx]
									if l.IsDisabled {
										if idx > 0 {
											s.NextBuffer[idx] = s.LayerData[idx-1]
										} else {
											s.NextBuffer[idx] = s.LayerData[idx]
										}
										continue
									}
									var input *Tensor[T]
									if l.IsRemoteLink {
										tIdx := n.GetIndex(l.TargetZ, l.TargetY, l.TargetX, l.TargetL)
										input = s.LayerData[tIdx]
									} else if idx > 0 {
										input = s.LayerData[idx-1]
									} else {
										input = s.LayerData[0]
									}
									if input != nil {
										pre, post := DispatchLayer(l, input, nil)
										s.NextBuffer[idx] = post
										if captureHistory {
											s.mu.Lock()
											currentIn[idx] = input
											currentPre[idx] = pre
											s.mu.Unlock()
										}
									}
								}
							}
						}
					}
				}(zTile, zEnd, yTile, yEnd, xTile, xEnd)
			}
		}
	}
	wg.Wait()

	for i := range s.LayerData {
		s.LayerData[i] = s.NextBuffer[i]
	}
	if captureHistory {
		s.HistoryIn = append(s.HistoryIn, currentIn)
		s.HistoryPre = append(s.HistoryPre, currentPre)
	}
	s.StepCount++
	return time.Since(start)
}

// StepBackward propagates gradients backward through the step history.
// It walks backward through clock cycles, accurately routing gradients to their source coordinates.
func StepBackward[T Numeric](n *VolumetricNetwork, s *StepState[T], gradOutput *Tensor[T]) (gradIn *Tensor[T], layerGradients [][2]*Tensor[T], err error) {
	s.mu.RLock()
	defer s.mu.RUnlock()

	if len(s.HistoryIn) == 0 {
		return nil, nil, nil
	}

	numSteps := len(s.HistoryIn)
	numLayers := len(n.Layers)

	layerGradients = make([][2]*Tensor[T], numLayers)

	// gradBuffers holds the current gradient "budget" for each coordinate.
	// In the step mesh, a coordinate's gradOutput is the sum of gradients
	// from all coordinates that consumed its output in the previous cycle.
	gradBuffers := make([]*Tensor[T], numLayers)

	// Initialize final layer with external gradient
	gradBuffers[numLayers-1] = gradOutput

	// Walk backwards through time steps (The Temporal dimension)
	for step := numSteps - 1; step >= 0; step-- {
		stepIn := s.HistoryIn[step]
		stepPre := s.HistoryPre[step]

		// Next cycle's gradient accumulation buffer
		nextGradBuffers := make([]*Tensor[T], numLayers)

		// Walk backwards through spatial coordinates
		for idx := numLayers - 1; idx >= 0; idx-- {
			l := &n.Layers[idx]
			if l.IsDisabled {
				// Pass gradient back to source
				if gradBuffers[idx] != nil {
					accumulateMeshGrad(n, nextGradBuffers, idx, gradBuffers[idx])
				}
				continue
			}

			input := stepIn[idx]
			pre := stepPre[idx]
			currentGrad := gradBuffers[idx]

			if input == nil || pre == nil || currentGrad == nil {
				continue
			}

			// DISPATCH BACKWARD
			gIn, gW := DispatchLayerBackward(l, currentGrad, input, nil, pre)

			// Accumulate weight gradients for this layer
			if layerGradients[idx][1] == nil {
				layerGradients[idx] = [2]*Tensor[T]{gIn, gW}
			} else if gW != nil {
				for i := range layerGradients[idx][1].Data {
					layerGradients[idx][1].Data[i] += gW.Data[i]
				}
			}

			// Route gIn back to the coordinate that supplied the input for this layer
			if gIn != nil {
				accumulateMeshGrad(n, nextGradBuffers, idx, gIn)
			}
		}

		// Carry gradBuffers to the next (earlier in time) step
		gradBuffers = nextGradBuffers
	}

	return gradBuffers[0], layerGradients, nil
}

// StepApplyTween bridges StepState with tween (neural target propagation).
// It uses the core 'Gap-Bridging' logic to update weights across the volumetric mesh.
func StepApplyTween[T Numeric](n *VolumetricNetwork, s *StepState[T], globalTarget *Tensor[T], lr float32) {
	s.mu.Lock()
	defer s.mu.Unlock()

	// 1. Initialize or update TweenState
	if s.TweenState == nil {
		config := DefaultTweenConfig()
		config.UseChainRule = false // Default to gap-based for step mesh o_O
		s.TweenState = NewTweenState[T](n, config)
	}

	// 2. Capture current mesh state as Forward Activations
	// Note: s.LayerData[idx] is the output of layer idx.
	// Tween expects s.ForwardActs[0] to be input, s.ForwardActs[1] to be output of Layer 0, etc.
	if len(s.LayerData) > 0 {
		s.TweenState.ForwardActs[0] = s.lastInput
		for i := 0; i < len(s.LayerData); i++ {
			s.TweenState.ForwardActs[i+1] = s.LayerData[i]
		}
	}

	// 3. SECONARY: If history is available, we could do BPTT-style tween passes,
	// but for now, we do steady-state grid refinement.

	// Use the standard tween backward (sequential index assuming)
	// or implement mesh-aware variant.
	// Since n.Layers is the linear order of the grid, the standard backward
	// usually works if the grid is designed sequentially.
	TweenBackward(n, s.TweenState, globalTarget)

	// 4. Update Diagnostics
	s.TweenState.CalculateLinkBudgets()

	// 5. Apply Gaps (Neural Weight Mutation)
	ApplyTweenGaps(n, s.TweenState, lr)
}

// accumulateMeshGrad finds the source for layer 'idx' and adds 'grad' to its entry in 'buffers'.
func accumulateMeshGrad[T Numeric](n *VolumetricNetwork, buffers []*Tensor[T], idx int, grad *Tensor[T]) {
	l := &n.Layers[idx]
	var sourceIdx int

	if l.IsRemoteLink {
		sourceIdx = n.GetIndex(l.TargetZ, l.TargetY, l.TargetX, l.TargetL)
	} else if idx > 0 {
		sourceIdx = idx - 1
	} else {
		sourceIdx = 0 // Input site
	}

	if buffers[sourceIdx] == nil {
		buffers[sourceIdx] = grad.Clone()
	} else {
		for i := range buffers[sourceIdx].Data {
			buffers[sourceIdx].Data[i] += grad.Data[i]
		}
	}
}
