...
func Plan9SimdForwardForLayer(t LayerType) bool {
	...
	switch t {
	case LayerDense, LayerSwiGLU, LayerMultiHeadAttention, LayerCNN1, LayerCNN2, LayerCNN3, LayerRNN, LayerLSTM, LayerEmbedding, LayerResidual:
		return true
	default:
		return false
	}
...
// SetSimdForward enables Plan 9 SIMD forward (Dense, SwiGLU, MHA, CNN1, CNN2, CNN3, RNN, LSTM, Embedding, Residual) on the network and every layer.
func (n *VolumetricNetwork) SetSimdForward(enabled bool) {
	...
	simd.SetInt8DotSimdForward(enabled)
}
