...
func runCNN1Example() bool {
	...
	for _, tc := range cnn1AllDTypes {
		...
		cnn1PrintSaveRow(tc.name, before)
		...
		learned := cnn1TrainingOK(res.LossHistory[0], lossAfter, tc.dtype)

		ck := cnn1SaveCheckpoint(net, tc.name)
		if ck != "" {
			fmt.Printf("  Checkpoint: %s\n", ck)
		}

		after := cnn1CheckSaveReload(net, input, target, tc, cnn1PhaseAfter, lossAfter)
		cnn1PrintSaveRow(tc.name, after)

		ok := before.pass && after.pass && learned
		fmt.Printf("  Loss %.6f → %.6f | learned=%v | overall=%s\n",
			res.LossHistory[0], lossAfter, learned, cnn1Mark(ok))
		if ok {
			passed++
		} else {
			failed++
	...
	fmt.Printf("\nCNN1 summary: %d passed, %d failed (of %d dtypes)\n", passed, failed, len(cnn1AllDTypes))
	return failed == 0
}
