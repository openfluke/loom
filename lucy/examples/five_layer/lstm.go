...
func runLSTMExample() bool {
	...
	for _, tc := range lstmAllDTypes {
		...
		lstmPrintSaveRow(tc.name, before)
		...
		learned := lstmTrainingOK(res.LossHistory[0], lossAfter, tc.dtype)

		ck := lstmSaveCheckpoint(net, tc.name)
		if ck != "" {
			fmt.Printf("  Checkpoint: %s\n", ck)
		}

		after := lstmCheckSaveReload(net, input, target, tc, lstmPhaseAfter, lossAfter)
		lstmPrintSaveRow(tc.name, after)

		ok := before.pass && after.pass && learned
		fmt.Printf("  Loss %.6f → %.6f | learned=%v | overall=%s\n",
			res.LossHistory[0], lossAfter, learned, lstmMark(ok))
		if ok {
			passed++
		} else {
			failed++
	...
	fmt.Printf("\nLSTM summary: %d passed, %d failed (of %d dtypes)\n", passed, failed, len(lstmAllDTypes))
	return failed == 0
}
