...
func runRNNExample() bool {
	...
	for _, tc := range rnnAllDTypes {
		...
		rnnPrintSaveRow(tc.name, before)
		...
		learned := rnnTrainingOK(res.LossHistory[0], lossAfter, tc.dtype)

		ck := rnnSaveCheckpoint(net, tc.name)
		if ck != "" {
			fmt.Printf("  Checkpoint: %s\n", ck)
		}

		after := rnnCheckSaveReload(net, input, target, tc, rnnPhaseAfter, lossAfter)
		rnnPrintSaveRow(tc.name, after)

		ok := before.pass && after.pass && learned
		fmt.Printf("  Loss %.6f → %.6f | learned=%v | overall=%s\n",
			res.LossHistory[0], lossAfter, learned, rnnMark(ok))
		if ok {
			passed++
		} else {
			failed++
	...
	fmt.Printf("\nRNN summary: %d passed, %d failed (of %d dtypes)\n", passed, failed, len(rnnAllDTypes))
	return failed == 0
}
