...
async function runGridScatterTraining() {
    ...
    console.log("Task: 3 agents learn to collaborate for binary classification\n");
    ...
    console.log(`Training for ${config.Epochs} epochs with learning rate ${config.LearningRate.toFixed(3)}`);
    ...
    console.log("\n✅ Training complete!");
    console.log(`Training time: ${((endTime - startTime) / 1000).toFixed(2)} seconds`);
    console.log(`Initial Loss: ${trainingData.LossHistory[0].toFixed(6)}`);
    console.log(`Final Loss: ${trainingData.FinalLoss.toFixed(6)}`);
    const improvement = (((trainingData.LossHistory[0] - trainingData.FinalLoss) /
    ...
    for (let i = 0; i < 4; i++) {
        const testResult = agentNetwork.ForwardCPU(JSON.stringify([batches[i].Input]));
        ...
        console.log(`Sample ${i}: [${pred[0].toFixed(3)}, ${pred[1].toFixed(3)}] → ` +
    ...
    const savedModelResult = agentNetwork.SaveModelToString(JSON.stringify(["grid_scatter_test"]));
    ...
    const reloadedNetwork = globalThis.loadLoomNetwork(savedModel, "grid_scatter_test");
    console.log("✓ Model loaded"); // Test predictions with reloaded model
    console.log("\nVerifying predictions match:");
    let allMatch = true;
    for (let i = 0; i < 4; i++) {
        const testResult = reloadedNetwork.ForwardCPU(JSON.stringify([batches[i].Input]));
        const testParsed = JSON.parse(testResult);
        const pred = testParsed[0];
        ...
        console.log(`Sample ${i}: [${pred[0].toFixed(3)}, ${pred[1].toFixed(3)}] (diff: ${maxDiff.toExponential(2)}) ${match ? "✓" : "✗"}`);
    }
    if (allMatch) {
